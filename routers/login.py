...
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = 60 #daqiqada
...
def get_current_user(db: Session = Depends(database), token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        raise credentials_exception
    user = db.query(UsersM).where(UsersM.username == token_data.username).first()
    if user is None:
        raise credentials_exception
    return user
...
async def login_for_access_token(db: Session = Depends(database), form_data: OAuth2PasswordRequestForm = Depends()):
    user = db.query(UsersM).filter(UsersM.username == form_data.username).first()
    if user:
    ...
    db.query(UsersM).filter(UsersM.id == user.id).update({
        UsersM.token: access_token
    })
    db.commit()
...
async def refresh_token(
    ...
    user = db.query(UsersM).where(UsersM.token == token).first()
    if user is None:
        raise HTTPException(
            status_code=400,
            detail="Token error",
